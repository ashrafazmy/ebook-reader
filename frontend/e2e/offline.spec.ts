...
test('offline player follows adjacent order, exposes gaps and selects exact downloaded versions with matching text and progress', async ({ page, context }, testInfo) => {
  ...
  await page.getByLabel('Chapters for playing book').selectOption('v3');
  ...
  await page.getByLabel('Chapters for playing book').selectOption('v0');
  ...
  await page.getByLabel('Chapters for playing book').selectOption('v1');
  await expect(page.locator('.playback-dock')).toContainText('not available offline');
  await expect(page.getByLabel('Chapters for playing book').locator('option[value="v1"]')).toHaveCount(0);
  await expect(page).toHaveURL(/download=v0/);
